...
def GetMetrics(y_pred, y_test, threshold):
    ...
    re = tp/(tp + fn)
    f1 = 2*((pr*re)/(pr+re))
    return {'Threshold': threshold, 'TN': tn, 'FP': fp, 'FN': fn, 'TP': tp, 'ACC': acc, 'PR': pr, 'RE': re, 'F1': f1, 'AUC': roc_auc, 'Erro': 1 - acc}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--Model_1', type=str, help='Model_1', required = True)
    parser.add_argument('--Model_2', type=str, help='Model_2', required = True)

    args = parser.parse_args()

    test_name = 'train_' + args.Model_1 + '_test_' + args.Model_2

    ##############################################################################
    #    Cria a pasta para armazenar os resultados e artefatos se não existir    #
    ##############################################################################
    if not(os.path.isdir(os.path.join('Resultados', 'CROSS_VALIDATION', 'Zero', test_name))):
        os.makedirs(os.path.join('Resultados', 'CROSS_VALIDATION', 'Zero', test_name))

    ##############################################################################
    #              Declaração dos parâmetros globais do código                   #
    ############################################################################## 
    PathTestData = os.path.join('Dataset', args.Model_2, 'test_set')
    ...
    #                           Carrega o modelo                                 #
    ##############################################################################
    model = keras.models.load_model(os.path.join('Resultados', args.Model_1, 'Zero', 'zero.h5'))

    ##############################################################################
    #              Obtem o melhor threshold obtido no treinamento                #
    ##############################################################################
    with open(os.path.join('Resultados', args.Model_1, 'Zero', 'TrainFinalMetrics.json')) as f:
    ...
    ####################################################################################
    FinalTrainMetrics = GetMetrics(y_pred_test, y_test_real, BestThreshold)
    FinalTrainMetrics['TempoPredict'] = PredictTime

    with open(os.path.join('Resultados', 'CROSS_VALIDATION', 'Zero', test_name, 'TestFinalMetrics.json'), 'w') as fp:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('Resultados', 'CROSS_VALIDATION', 'Zero', test_name, "Resultado.png"))


    ####################################################################################
    #                         Salva os artefatos do predict                            #
    ####################################################################################
    df_predict_test.to_csv(os.path.join('Resultados', 'CROSS_VALIDATION', 'Zero', test_name, 'TestArtifacts.csv'), sep = '\t', index = False)
