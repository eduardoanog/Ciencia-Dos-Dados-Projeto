...
if __name__ == "__main__":
    # Argument Parser
    parser = argparse.ArgumentParser(description='Treino do modelo do zero')
    ...
    ##############################################################################
    if not(os.path.isdir(os.path.join('Resultados', args.Dataset, 'Zero'))):
        os.makedirs(os.path.join('Resultados', args.Dataset, 'Zero'))
    ...
    ##############################################################################
    model = Sequential()
    model.add(Conv2D(32, (3, 3), input_shape = InputSize, activation = 'relu'))
    ...
    #                              Salva o modelo                               #
    #############################################################################
    model.save(os.path.join('Resultados', args.Dataset, 'Zero', 'zero.h5'))

    #############################################################################
    #                Plota o histórico de treinamento e validação               #
    #############################################################################
    PlotTrainHistory(history, os.path.join('Resultados', args.Dataset, 'Zero', 'TrainHistory.png'))

    hist_df = pd.DataFrame(history.history) 
    hist_df.to_csv(os.path.join('Resultados', args.Dataset, 'Zero', 'TrainHistory.csv'), sep = '\t')
    ...
    ####################################################################################
    PlotCurves(y_val_real, y_pred_val, os.path.join('Resultados', args.Dataset, 'Zero', 'ROCAUC.png'), os.path.join('Resultados', args.Dataset, 'Zero', 'MetricCurves.png'), BestThreshold)

    ####################################################################################
    #                        Obtem as métricas de validação                            #
    ####################################################################################
    FinalTrainMetrics = GetMetrics(y_pred_val, y_val_real, BestThreshold)
    FinalTrainMetrics['TempoTreinamento'] = TrainingTime

    with open(os.path.join('Resultados', args.Dataset, 'Zero', 'TrainFinalMetrics.json'), 'w') as fp:
    ...
    df_predict_artifact.to_csv(os.path.join('Resultados', args.Dataset, 'Zero', 'ValidationPredict.csv'), sep = '\t', index = False)
