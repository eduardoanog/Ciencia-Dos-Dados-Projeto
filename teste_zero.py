...
if __name__ == "__main__":
    ...
    #                           Carrega o modelo                                 #
    ##############################################################################
    model = keras.models.load_model(os.path.join('Resultados', args.Dataset, 'Zero', 'zero.h5'))

    ##############################################################################
    #              Obtem o melhor threshold obtido no treinamento                #
    ##############################################################################
    with open(os.path.join('Resultados', args.Dataset, 'Zero', 'TrainFinalMetrics.json')) as f:
    ...
    with open(os.path.join('Resultados', args.Dataset, 'Zero', 'TestFinalMetrics.json'), 'w') as fp:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('Resultados', args.Dataset, 'Zero', "Resultado.png"))


    ####################################################################################
    #                         Salva os artefatos do predict                            #
    ####################################################################################
    df_predict_test.to_csv(os.path.join('Resultados', args.Dataset, 'Zero', 'TestArtifacts.csv'), sep = '\t', index = False)
