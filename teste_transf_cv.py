...
if __name__ == "__main__":
    ...
    ##############################################################################
    if not(os.path.isdir(os.path.join('Resultados', 'CROSS_VALIDATION', 'Transf',  test_name, args.BaseModel))):
        os.makedirs(os.path.join('Resultados', 'CROSS_VALIDATION', 'Transf', test_name, args.BaseModel))
    ...
    #                           Carrega o modelo                                 #
    ##############################################################################
    model = keras.models.load_model(os.path.join('Resultados', args.Model_1, 'Transf', args.BaseModel, 'transf.h5'))

    ##############################################################################
    #              Obtem o melhor threshold obtido no treinamento                #
    ##############################################################################
    with open(os.path.join('Resultados', args.Model_1, 'Transf', args.BaseModel, 'TrainFinalMetrics.json')) as f:
    ...
    with open(os.path.join('Resultados', 'CROSS_VALIDATION', 'Transf',  test_name, args.BaseModel, 'TestFinalMetrics.json'), 'w') as fp:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('Resultados', 'CROSS_VALIDATION', 'Transf',  test_name, args.BaseModel, "Resultado.png"))


    ####################################################################################
    #                         Salva os artefatos do predict                            #
    ####################################################################################
    df_predict_test.to_csv(os.path.join('Resultados', 'CROSS_VALIDATION', 'Transf',  test_name, args.BaseModel, 'TestArtifacts.csv'), sep = '\t', index = False)
